# -*- coding: utf-8 -*-
"""Andre Data Analysis.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1IRbSuSQGFJRDjakpQUTOSN-byvdmFVQb
"""
...
'''Este código gera um histograma que exibe a distribuição do número de plantões por idade. Cada barra
representa a quantidade de plantões realizados por pessoas em uma faixa etária específica, permitindo
visualizar como a frequência de plantões varia entre as idades. O eixo x mostra as idades,
enquanto o eixo y indica a contagem de plantões para cada idade.'''
...
'''Este código gera um gráfico de linhas que exibe a frequência mensal média de plantões por idade. A média é calculada
dividindo o número total de plantões por idade pelo número de meses. No gráfico, o eixo x
representa as idades, enquanto o eixo y mostra a frequência média mensal de plantões para cada idade.'''

#Histograma da Idade 1 e 2#
...
'''Este código gera um histograma que mostra o número de médicos muito ativos por idade. Um médico é considerado
"muito ativo" se realizar mais de 3 plantões por semana, em média, no último ano. O filtro
inicial seleciona os médicos com atividades nos últimos 365 dias, e a frequência
mínima de 3 plantões por semana (ou 48 plantões por ano) é aplicada para
identificar esses profissionais. No gráfico, o eixo x representa as idades,
enquanto o eixo y mostra o número de médicos muito ativos em cada faixa etária'''
...
'''Este código gera um histograma que mostra o número de médicos ativos por idade. Um médico é considerado
"ativo" se realizar mais de 3 plantões por semana, em média, no último ano. O filtro
inicial seleciona os médicos com atividades nos últimos 365 dias, e a frequência
mínima de 3 plantões por semana (ou 48 plantões por ano) é aplicada para
identificar esses profissionais. No gráfico, o eixo x representa as idades,
enquanto o eixo y mostra o número de médicos ativos em cada faixa etária'''

#Histograma da Idade 3#
...
'''Este código gera um histograma que mostra o número de médicos inativos por idade. Um médico é considerado
"inativo" se tiver realizado menos de 3 plantões mensais, em média, nos últimos 6 meses (ou seja, menos de 18 plantões no período).
No gráfico, o eixo x representa as idades, enquanto o eixo y
mostra o número de médicos inativos em cada faixa etária.'''

#Frequencia x Numero de Medicos#
...
plt.tight_layout()
plt.show()
'''Este código gera um histograma que exibe a distribuição da frequência mensal de plantões
entre médicos ativos no último ano. A definição de "médico ativo" aqui é qualquer
médico que realizou, em média, pelo menos 1 plantão por mês ao longo do último ano.
No eixo x está a frequência mensal de plantões, enquanto o eixo y indica o número de médicos que se enquadram em cada faixa de frequência.'''